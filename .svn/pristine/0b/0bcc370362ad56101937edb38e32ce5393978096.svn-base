			        	...
			        	<table class="table table-striped table-bordered table-hover dataTable no-footer" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info">
				            ...
				            <tbody>
				            	<c:forEach var="item" items="${list }">
				            		<tr>
				            			<td><input id="${item.tid}" type="checkbox" name="teacher" class="required" onclick="getId(${item.tid});"></td>
										<td>${item.name }</td>
										<td>${item.account }</td>
										<td>${item.password }</td>
...
function doPage(n) {		
	$.post("chooseTeacher",{pageNow : n}, function(data) {
		$("#chooseTeacher").html(data);
...
function aa(){
	var idd = document.getElementsByName("teacher");
	var ids = document.getElementById("ids").value;//隐藏域里的所有选中的id，字符串
	var arr = ids.split(",");
...
$("#confirm").click(function() {
	$.ajax({
		...
		cache : false,
		type : "post",
		data : "groupName="+$("#name").val()+"&ids="+$("#ids").val(),
		url : "insertGroupTeacher",		
		success : function() {
			$.post("clearSession");
