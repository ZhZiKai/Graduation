...
<body>
	<div id="wrap" class="container">
		...
		<h4>东北林业大学信息与计算机工程学院</h4>
		<h4>毕业生论文管理系统</h4>
		<form id="studentForm">
			<div class="form-group number">
				<input id="student-number-input"  name="account" type="text" class="form-control input-lg" placeholder="请输入账号">
			</div>
			<div class="form-group password">
				<input id="student-password-input" name="password"  type="password" class="form-control input-lg" placeholder="请输入密码">
			</div>
			<button id="student-submit-button" type="button"  class="btn btn-success btn-lg submit-button" onclick="loginTeacher()">登录</button>
	...
	<script type="text/javascript">
			function loginTeacher(){
				var account = $("#student-account-input").val();
				var password = $("#student-password-input").val();
				if(account == "" || password == "") {
				...
				$.ajax({
					...
					url : '${pageContext.request.contextPath}/comm/login_student',
					success : function (json) {
						if(json.statusCode == '200') {
							window.location = '${pageContext.request.contextPath}/yy/student/index';
						}else{
							alert(json["message"]);
							$(".form-group").addClass("has-error");
						}
